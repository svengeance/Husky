...
namespace Husky.Internal.Generator
{
    public sealed class SourceBuilder
    {
        ...
        public IDisposable Block(string? line = null, string? postFix = null)
        {
            BlockOpen(line);
            return new DisposableAction(() => BlockClose(postFix));
        }

        public SourceBuilder BlockClose(string? postFix = null, bool appendLine = true)
        {
            IndentDown();
            if (appendLine)
                AppendIndentedLine($"}}{postFix}");
            else
                AppendIndented($"}}{postFix}");
